...
#pragma config(Motor,  port4,           Motor_FW1,     tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port5,           Motor_FW2,     tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port6,           Motor_FW3,     tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port7,           Motor_FW4,     tmotorVex393_MC29, openLoop)
...
FwMotorEncoderGet()
{
	return( SensorValue[ QuadEncoder1 ] );
}
...
task main()
{
	...
	while(1)
	{
		...
		{
			if( flywheel.v_current + 50 < 2600)
			{
				FwVelocitySet( &flywheel, flywheel.target+50, flywheel.drive+0.02 );
				writeDebugStreamLine("Current Flywheel speed %f", flywheel.target);
				writeDebugStreamLine("Current Flywheel drive %f", flywheel.drive);
			}
			wait1Msec(1000);
		}
		else if( vexRT[ Btn8D ] == 1 )
		{
			if( flywheel.v_current - 50 > 200)
			{
				FwVelocitySet( &flywheel, flywheel.target-50, flywheel.drive-0.02 );
				writeDebugStreamLine("Current Flywheel speed %f", flywheel.target);
				writeDebugStreamLine("Current Flywheel drive %f", flywheel.drive);
